...
class DCNv3_PyTorch(nn.Module):
    def __init__(self, in_channels, out_channels, kernel_size=1, stride=1, groups=1, dilation=1):
        ...
        self.ln = nn.LayerNorm(out_channels, eps=1e-6)
        self.gelu = nn.GELU()
...
class DConv(nn.Module):
    ...
    def __init__(self, c1, c2, k=1, s=1, p=None, g=1, d=1, act=True):
        ...
        super().__init__()
        # print("conv")
        self.depthwise = nn.Conv2d(c1, c2, 3, s, autopad(3, p, d), groups=c1, dilation=d, bias=False)
        self.pointwise = nn.Conv2d(c1, c2, 1, s, autopad(1, p, d), bias=False)
        self.bn = nn.BatchNorm2d(c2)
        self.act = self.default_act if act is True else act if isinstance(act, nn.Module) else nn.Identity()
    ...
    def forward_fuse(self, x):
        """Perform transposed convolution of 2D data."""
        return self.simam(self.act(self.pointwise(self.depthwise(x))))
    
class DConv_with_ca(nn.Module):
    """Standard convolution with args(ch_in, ch_out, kernel, stride, padding, groups, dilation, activation)."""
    ...
    def forward(self, x):
        """Apply convolution, batch normalization and activation to input tensor."""
        return self.ca(self.act(self.bn(self.pointwise(self.depthwise(x)))))
